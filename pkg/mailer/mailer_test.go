...
func TestSMTPMailer_LocalizedSubjects(t *testing.T) {
	...
	cases := []struct {
	...
	}{
		{
			...
			expectedSubject: "Subject: Votre code d'authentification Broadsheet",
		},
		{
			...
			expectedSubject: "Subject: Vous avez été invité à rejoindre Mon Espace sur Broadsheet",
		},
		{
...
func TestSMTPMailer_UnknownLanguageFallsBackToEnglish(t *testing.T) {
	...
	if !strings.Contains(logOutput, "Subject: Your Broadsheet authentication code") {
...
func TestConsoleMailer_LocalizedContent(t *testing.T) {
	...
	expectedStrings := []string{
		...
		"Bonjour,",
		"Votre code d'authentification pour Broadsheet est :",
		"123456",
		"L'équipe Broadsheet",
	}
	for _, expected := range expectedStrings {
