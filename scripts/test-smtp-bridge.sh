...
# Default values
SMTP_SERVER="${SMTP_SERVER:-localapi.broadsheet.local}"
...
if [ -z "$API_EMAIL" ] || [ -z "$API_KEY" ] || [ -z "$WORKSPACE_ID" ]; then
    ...
    echo "  SMTP_SERVER=localapi.broadsheet.local    # SMTP server address"
...
elif [ -f "./dev-certs/localapi.broadsheet.local.cert.pem" ]; then
    TLS_CA_PATH="--tls-ca-path ./dev-certs/localapi.broadsheet.local.cert.pem"
    print_info "Using TLS certificate: ./dev-certs/localapi.broadsheet.local.cert.pem"
else
